...
// There is a bug in the OS preventing the Effects detection (Noise SUppression
// and AEC) to work for Win10 Phones. The bug is severe enough that it's not
// only the detection that doesn't work but the activation of the effect. For
// Windows phone (until the bug is solved at the OS level, it will return false,
// and the software AEC will be used
#if 0
